...
ggmap(eniMap) +
  stat_summary_2d(geom = "tile",bins = 30, data=race, aes(x = long, y = lat, z = eni), alpha=0.5) +
...
scoreENI <- lm(academicScore ~ eni + pctBlackHispanic, data = school_clean)
summary(scoreENI)
plot(scoreENI)
```
...
#Racial Demographic Distribution
ggplot(score, aes(x = count, fill = race)) +
  geom_bar() +
  facet_wrap(race ~ .) +
...
ggmap(eniMap) +
  ...
  labs( x="",
        y ="")
```
### Recommendation 1: Geolocation Strategy
Based on the information above, we can see there is a clear corridor of high scores in high need, URM schools. PASSNYC can concentrate marketing and outreach efforts to this area to begin raising awareness for their solutions so to gain the buy-in from parents, teachers, and administrators.
...
ggplot(hipo, aes(as.factor(district))) +
  geom_bar(stat = "count") +

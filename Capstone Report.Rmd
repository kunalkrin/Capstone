...
num.school <- school_clean%>%
  filter(!is.na(took))%>%
  select(-income, -City, -address, -DBN, -registered, -yield, -took, -enroll,-regPct ,-commSchool, -school, -lat, -long, -zip, -district, -elaAsian, -mathAsian, -mathWhite, -pctELAAsian, -elaAsian, -pctMathAsian, -pctMathWhite, -(contains("rating")))

scaled.school<- (scale(num.school))
schoolPr <- prcomp(scaled.school)

schoolPr
plot(schoolPr, type = "lines")
pamk<- pam(scaled.school, 5)
plot(silhouette(pamk))
biplot(schoolPr)
```

So based on our plot, we should either use 2 or 5 clusters, but based on our silhouette plot, the reality is with either 2 or 5 clusters, our structure to our clusters in non-existent. We also graphed it out, and this is also the case. This is definitely as a result of a lack of data points. 

Despite that fact, let's see how our schools would be clustered and how much variability they can explain.
...
for (i in 1:k){
  wss[i] <-
    sum(kmeans(scaled.school, centers=i)$withinss)
}
plot(1:15,
     wss,
     type="b",
...
clustDist<- dist(scaled.school, method = "euclidean")
clustDist<- hclust(clustDist)
plot(clustDist)


final <- kmeans(scaled.school, centers = 4, nstart = 25)
final

plot(num.school[c("eni","tookPct")], col =  final$cluster)

```
So when we use 2 cluters, we can only explain about 22% of the variability which is weak, but with 4 we can explain about 50% which is decent.

When we graph the clusters out, there are some distinct clusters  in eni and who took the test. Although, there is overlap and isn't particularly helpful to us.
...
corrplot(cor(reduced_Data), type = "upper", method = "square")
...
Based on the feature selection above, it looks like our optimal model to predict the number of students who take th SHSAT depends on community strength, attendance, and the number of ELA 4 scores schools receive. How interesting.

```{r}
### Model Prediction (filling in the took %)
set.seed(1234)

modelml <- lm(tookPct~ pctCommunity + pctAttend + elaAll, data = num.school)
summary(modelml)

predictTest <- predict(modelml, newdata = num.school)

postResample(pred = predictTest, obs = num.school$tookPct)
...
#remove nzv
nzv_cols <- nearZeroVar(schoolPredict)
if(length(nzv_cols) > 0) schoolPredict <- schoolPredict[, -nzv_cols]

#remove high corr
correlation = cor(schoolPredict, -tookFactor)
hc2 = findCorrelation(correlation, cutoff=0.5)
hc2 = sort(hc2)
schoolPredict = schoolPredict[,-c(hc2)]

#elbow and silhouette
fviz_nbclust(schoolPredict, kmeans, method = "wss")
fviz_nbclust(schoolPredict, kmeans, method = "silhouette")
final <- kmeans(schoolPredict, 2, nstart = 25)
print(final)
fviz_cluster(final, data = schoolPredict)
...
schoolPredict$tookFactor <- as.factor(schoolPredict$tookFactor)
summary(schoolPredict$tookFactor)


set.seed(1234)
model <- randomForest(tookPct ~ . -tookFactor, data = train)
pred <- predict(model, newdata = test)
table(pred, test$tookPct)


trainIndex <- createDataPartition(na.omit(schoolPredict$tookPct), p = .8, 
                                  list = FALSE, 
                                  times = 1)
head(trainIndex)
...
# train the model
model <- train(tookFactor~., data=schoolPredict, trControl=train_control, method="nb")
# summarize results
print(model)
plot(model)

#Bayes gives us a 90% accuracy.
pred1<-predict(model, newdata = test)
table(pred1, test$tookFactor)



#Random Forest gives us a 99.6% accuracy.
fit<- randomForest(tookFactor~.,  data = train)
pred <- predict(fit, newdata = test)
table(pred, test$tookFactor)
...
```
After testing the model, it's not bad: we get an R-squared of .566. Now let's use this model to predict other schools' turnouts for the SHSAT.

#Recommendation 3: Target the schools using this specialized model.

Based on the model we created, we can predict which schools will have low turnouts based on the model. Note that the model is using a small data set and won't be the most accurate, but it does give us a pretty good sense of where NYCPASS's programs will make an impact.

...
#Heatmap of ENI
ggmap(map) +
  stat_summary_2d(geom = "tile",bins = 15, data=race, aes(x = long, y = lat, z = eni), alpha=0.5) +
...
```{r echo=F, message=FALSE, warning=FALSE, paged.print=TRUE}
ggplot(race, aes(x = eni, y = percent, col = race)) +
  geom_point() +
  facet_wrap(.~race)
...
score <- gather(school_clean, race, count, c(elaBlack, elaHispanic, mathBlack, mathHispanic), factor_key=TRUE)
score <- score %>%
...
#Racial Demographic Distribution
ggplot(score, aes(y = count, x = eni, col = race)) +
  geom_point() +
  facet_grid(. ~ race) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
ggmap(map) +
  stat_summary_2d (geom = "tile", bins = 10, data = score, aes(x = long, y = lat, z = count), alpha = 0.6) +
  scale_fill_gradient(low = "black", high = "green", guide = guide_legend(title = "Density of 4 Scores")) +
  labs( x="",
        y ="")
ggplot(score, aes(x=reorder(City,count, length))) +
...
scoreENImodel <- lm(academicScore ~ pctBlackHispanic + eni + p, data = school_clean)
summary(scoreENImodel)
...
final <- kmeans(scaled.school, centers = 2, nstart = 25)
final
...
# train the model
fit<- randomForest(factor(tookPct)~., data=num.school)
...
SSE <- sum((num.school$tookPct - predictTest)^2)
SST <- sum((predictTest - mean(num.school$tookPct))^2)
